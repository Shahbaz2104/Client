...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (!is_unique_slug($slug, 'products')) $errors[] = 'Slug already exists';

        if (empty($errors)) {
            $stmt = $pdo->prepare("
                INSERT INTO products (name, slug, short_description, description, category_id, price, image_url, status, sort_order, meta_title, meta_description) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $name, $slug, $short_description, $description, $category_id, $price, $image_url ?: null, $status, $sort_order, $meta_title ?: null, $meta_description ?: null
            ]);
                        ...
                        <div>
                            ...
                            >
                                ...
                                <?php foreach ($categories as $category): ?>
                                    <option value="<?= e((string)$category['id']) ?>" <?= old('category_id') == $category['id'] ? 'selected' : '' ?>>
                                        <?= e($category['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
