...
$pageTitle = 'Edit Gallery Item: ' . $item['title'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("
                UPDATE gallery 
                SET title = ?, image_url = ?, description = ?, sort_order = ?, status = ? 
                WHERE id = ?
            ");
            $stmt->execute([
                $title, $image_url, $description, $sort_order, $status, $id
            ]);
            ...
            <div class="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
                ...
                <?php if ($item['image_url']): ?>
                    <div class="mb-6 rounded-xl overflow-hidden border border-slate-200">
                        <img src="<?= e($item['image_url']) ?>" alt="<?= e($item['title']) ?>" class="w-full h-48 object-cover">
                    </div>
                <?php endif; ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="sort_order"
                                name="sort_order"
                                value="<?= old('sort_order', (string)$item['sort_order']) ?>"
                                min="0"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
