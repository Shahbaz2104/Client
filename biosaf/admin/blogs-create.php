...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        } elseif (!is_unique_slug($slug, 'blogs')) {
            $errors[] = 'Slug already exists. Please choose a unique one.';
        }

        if (empty($errors)) {
            $stmt = db()->prepare("
                INSERT INTO blogs (title, slug, excerpt, content, author, status, meta_title, meta_description, featured_image) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $title, $slug, $excerpt, $content, $author, $status, $meta_title ?: null, $meta_description ?: null, $featured_image ?: null
            ]);
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="author"
                                name="author"
                                value="<?= old('author', $admin['username']) ?>"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
