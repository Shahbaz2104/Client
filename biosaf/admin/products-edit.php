...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (!is_unique_slug($slug, 'products', $id)) $errors[] = 'Slug already exists';

        if (empty($errors)) {
            $stmt = $pdo->prepare("
                UPDATE products 
                SET name = ?, slug = ?, short_description = ?, description = ?, category_id = ?, price = ?, image_url = ?, status = ?, sort_order = ?, meta_title = ?, meta_description = ? 
                WHERE id = ?
            ");
            $stmt->execute([
                $name, $slug, $short_description, $description, $category_id, $price, $image_url ?: null, $status, $sort_order, $meta_title ?: null, $meta_description ?: null, $id
            ]);
...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <div class="lg:ml-64 min-h-screen flex flex-col">
        ...
        <main class="flex-1 p-6">
            <div class="mb-6">
                <a href="<?= BASE_URL ?>/admin/products.php" class="inline-flex items-center gap-2 text-slate-500 hover:text-slate-700 transition-colors text-sm mb-4">
            ...
            <div class="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <?php foreach ($categories as $category): ?>
                                    <option value="<?= e((string)$category['id']) ?>" <?= old('category_id', (string)$product['category_id']) == $category['id'] ? 'selected' : '' ?>>
                                        <?= e($category['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div>
                            <label for="image_url" class="block text-sm font-semibold text-slate-700 mb-2">Image URL</label>
                            <input
                                type="url"
                                id="image_url"
                                name="image_url"
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="sort_order"
                                name="sort_order"
                                value="<?= old('sort_order', (string)$product['sort_order']) ?>"
                                min="0"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
                    ...
                    <div class="pt-4 flex items-center gap-4">
                        ...
                        <a href="<?= BASE_URL ?>/admin/products.php" class="px-8 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-all">
                            Cancel
                        </a>
                    </div>
                </form>
