...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        } elseif (!is_unique_slug($slug, 'services', $id)) {
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("
                UPDATE services 
                SET name = ?, slug = ?, description = ?, short_description = ?, icon = ?, division_id = ?, sort_order = ?, status = ?, meta_title = ?, meta_description = ?
                WHERE id = ?
            ");
            $stmt->execute([
                $name,
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <?php foreach ($divisions as $division): ?>
                                    <option value="<?= e((string)$division['id']) ?>" <?= old('division_id', (string)$service['division_id']) == $division['id'] ? 'selected' : '' ?>>
                                        <?= e($division['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="sort_order"
                                name="sort_order"
                                value="<?= old('sort_order', (string)$service['sort_order']) ?>"
                                min="0"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
