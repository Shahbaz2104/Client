...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        } elseif (!is_unique_slug($slug, 'divisions', $id)) {
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("
                UPDATE divisions 
                SET name = ?, slug = ?, description = ?, short_description = ?, icon = ?, sort_order = ?, status = ?, meta_title = ?, meta_description = ?
                WHERE id = ?
            ");
            $stmt->execute([
                $name,
...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <div class="lg:ml-64 min-h-screen flex flex-col">
        ...
        <main class="flex-1 p-6">
            <div class="mb-6">
                <a href="<?= BASE_URL ?>/admin/divisions.php" class="inline-flex items-center gap-2 text-slate-500 hover:text-slate-700 transition-colors text-sm mb-4">
            ...
            <div class="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="sort_order"
                                name="sort_order"
                                value="<?= old('sort_order', (string)$division['sort_order']) ?>"
                                min="0"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input
                                ...
                                value="<?= old('short_description', $division['short_description']) ?>"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input
                                ...
                                id="meta_title"
                                name="meta_title"
                                value="<?= old('meta_title', $division['meta_title']) ?>"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            >
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea
                                ...
                                rows="3"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
                            ><?= old('meta_description', $division['meta_description']) ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 flex items-center gap-4">
                        ...
                        <a href="<?= BASE_URL ?>/admin/divisions.php" class="px-8 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-all">
                            Cancel
                        </a>
                    </div>
                </form>
