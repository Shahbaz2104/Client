...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        } elseif (!is_unique_slug($slug, 'industries')) {
        ...
        if (empty($errors)) {
            $pdo = db();
            $stmt = $pdo->prepare("
                INSERT INTO industries (name, slug, description, icon, sort_order, status, meta_title, meta_description) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $name, $slug, $description, $icon ?: null, $sort_order, $status, $meta_title ?: null, $meta_description ?: null,
            ]);

            flash('success', 'Industry created successfully!');
