...
export default function Products() {
  ...
  const [productForm, setProductForm] = useState({ company: '', email: '', phone: '', message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!productForm.company || !productForm.email || !productForm.phone) return;
    ...
    try {
      const res = await fetch('/api/quote', {
        ...
        body: JSON.stringify({
          name: productForm.company,
          email: productForm.email,
          phone: productForm.phone,
          company: productForm.company,
          message: `Products requested: ${inquiryList.length > 0 ? inquiryList.map(i => i.title).join(', ') : productForm.message}`,
        }),
      });
      if (res.ok) {
        setProductToast({ type: 'success', message: 'Sourcing RFP submitted! Our procurement engineer will contact you shortly.' });
        setProductForm({ company: '', email: '', phone: '', message: '' });
        setInquiryList([]);
      } else {
        const err = await res.json();
        setProductToast({ type: 'error', message: err.error || 'Failed to submit.' });
      }
    } catch {
      setProductToast({ type: 'error', message: 'Network error. Please try again.' });
    } finally {
  ...
  const transferToRfp = () => {
    setShowDrawer(false);
    const items = inquiryList.map((item) => item.title).join('; ');
    setProductForm((prev) => ({ ...prev, message: items }));
    setTimeout(() => {
      document.getElementById('inquiry-scope')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      document.getElementById('inquiry-scope')?.focus();
    }, 100);
  };
      ...
      <div>
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <ScrollReveal className="bg-white rounded-3xl p-6 shadow-md mb-8 border border-gray-100">
              <div className="grid md:grid-cols-12 gap-4 items-center">
                ...
                <div className="md:col-span-7 flex flex-wrap gap-2 justify-start md:justify-end">
                  {(['all', 'instruments', 'glassware-reagents', 'testing-kits', 'lab-furniture'] as const).map((cat) => (
                    <button
                      key={cat}
                    ...
                    >
                      {cat === 'all' ? 'All Sourcing' : cat === 'instruments' ? 'Analytical Instruments' : cat === 'glassware-reagents' ? 'Glassware & Reagents' : cat === 'testing-kits' ? 'Testing & Diagnostics' : 'Lab Furniture'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-brand-primary rounded-[3rem] overflow-hidden shadow-2xl border border-white/5 relative">
                <div className="grid lg:grid-cols-12 items-stretch">
                  ...
                  <div className="lg:col-span-5 bg-white/5 border-l border-white/10 p-8 md:p-12 relative z-10 flex flex-col justify-center">
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                      ...
                      <div>
                        ...
                        <textarea
                          id="inquiry-scope"
                          required
                          value={inquiryList.length > 0 ? inquiryList.map(i => i.title).join('; ') : productForm.message}
                    ...
                    {productToast && (
                      <div className={`mt-4 p-4 rounded-xl text-xs font-bold text-center ${productToast.type === 'success' ? 'bg-emerald-950/80 text-emerald-400 border border-emerald-500/30' : 'bg-red-950/80 text-red-400 border border-red-500/30'}`}>
                        <p className="flex items-center justify-center gap-1.5">{productToast.message}</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {showDrawer && (
            ...
            >
              ...
              <motion.div
                ...
                className="relative h-full w-full sm:w-[450px] bg-brand-dark border-l border-white/10 flex flex-col justify-between p-6 shadow-2xl text-white"
              >
                <div>
                      ...
                      inquiryList.map((item) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            aria-label={`Remove ${item.title} from inquiry list`}
                          >
                            <svg
                              className="w-5 h-5"
                              fill="currentColor"
